...
- `migrations_pattern` <Type text="string" /> <MetaInfo text="optional" />
  - A glob pattern (relative to your Wrangler config file) used to discover migration files. Defaults to `${migrations_dir}/*.sql`.
  - Use this to opt in to nested layouts produced by ORMs like Drizzle (for example, `migrations/*/migration.sql`).
  - When `migrations_pattern` is set, `migrations_dir` must also be set, and the pattern must start with `${migrations_dir}/`. Each migration is recorded in the migrations table as a path relative to `migrations_dir`.
