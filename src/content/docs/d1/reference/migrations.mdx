...
- The pattern must start with `${migrations_dir}/`.
...
`wrangler d1 migrations create` only writes top-level files inside `migrations_dir`, so if your `migrations_pattern` only matches nested files (as with the Drizzle layout above), generate new migrations using your ORM's command (for example, `drizzle-kit generate`) instead.
